...
client.on('message', message => {
    if (!message.author.bot & message.content.substring(0, config.prefix.length) == config.prefix) { // If not a bot (to prevent infinite loops) and if there is the prefix
        ...
        switch (command[0]) {
            ...
            ///
            /// This stuff is EndBot only. Will have to be deleted when we will release the bot to everyone
            ///

            case 'rules':

                message.guild.members.fetch(message.author.id).then(guildUser => {
                    if (guildUser.hasPermission('MANAGE_CHANNELS')) {
                        message.channel.send(
                            new Discord.MessageEmbed()
                                .setColor('#EBA620')
                                .setTitle('About us')
                                .setDescription(
                                    'EndTech is an international semi-vanilla server centered around all technical aspects of Minecraft.\n' +
                                    'Launched in 1.14, currently running on 1.15.2 and updated upon each stable release, our main goal is to endgame everything, with technology avaible in the current version, as well as developing 1.13+ technical minecraft.\n' +
                                    'EndTech members take satisfaction in building farms, automation machinery, plugins and mods.\n' +
                                    'You share our goals and thoughts? Consider appying!'
                        ...
                        message.channel.send(
                            new Discord.MessageEmbed()
                                ...
                                .setDescription('Even if we aren\'t sticklers with rules, please use common sense and act as an adult.')
                                .addFields(
                                    {name: '**Verbal trashfires are to be avoided.**', value: 'In case of emergency, call @911.'},
                                    {name: '**Have a voice.**', value: 'Debate, but respect other members opinions and thoughts while doing so. Also, don\'t feed trolls.'},
                                    {name: '**Don\'t act hatefully.**', value: 'EndTech is about being inclusive, not exclusive. Don’t be oppressive over persecuted groups, don’t use the n-word or the f-slur.'},
                                    {name: '**Do not post immoral/graphic content.**', value: 'While wittiness is to some extent debatable, keep this for your homies.'}
                                )
                        )
            ...
            case 'discords-links':
                message.guild.members.fetch(message.author.id).then(guildUser => {
                    if (guildUser.hasPermission('MANAGE_CHANNELS')) {
                        let embed_ = helper.sendLinks(config["discord-links"].active)
                            .setTitle('Other technical discords')
                            .setDescription('The technical community is a large one, so we encourage you to join these servers to ehance your experience!')
                        let embed__ = helper.sendLinks(config["discord-links"]["technical-friendly"])
                            .setTitle('Technical-friendly servers')
                        let embed___ = helper.sendLinks(config["discord-links"].research)
                            .setTitle('Reasearch and discussion')

                        message.channel.send(embed_);
                        message.channel.send(embed__)
                        message.channel.send(embed___)
